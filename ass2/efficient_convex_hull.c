...
void separate_points( t_point *points, int num_point, t_point from, t_point to, t_point *s1, t_point *s2, int *n1, int *n2)
{
	...
	for (i = 0; i< num_point; i++){
		if(points[i].x == from.x && points[i].y== from.y) continue;
		if(points[i].x == to.x && points[i].y== to.y) continue;
...
{	
	...
	{	
		...
		if(*capacity == 0) {
			*capacity = 10;
			t_line *tmp = (t_line*)realloc(lines, ((*num_line + *capacity) * sizeof(t_line)));
			if (tmp != NULL) {
				lines = tmp;
			}
			else free(tmp);
	...
	// x1: set of points
	x1 = (t_point *)malloc( sizeof(t_point) * num_point);
	assert( x1 != NULL);

	// x2: set of points
	x2 = (t_point *)malloc( sizeof(t_point) * num_point);
	assert( x2 != NULL);
	
	// x0 : set of points
	x0 = (t_point *)malloc( sizeof(t_point) * num_point);
	assert( x0 != NULL);
	...
	for(i = 1; i< num_point;i++){
		if(abs(distance(a,b,c, points[far])) < abs(distance(a,b,c, points[i]))) {
			far = i;
		}
	}
	
	// 재귀 호출
	separate_points(points, num_point, p1, points[far], x1, x0, &n1, &n0);
	separate_points(points, num_point, points[far], pn, x2, x0, &n2, &n0);
	
	free( x0);

	lines = upper_hull( x1, n1, p1, points[far], lines, num_line, capacity);
	lines = upper_hull( x2, n2, points[far], pn, lines, num_line, capacity);
